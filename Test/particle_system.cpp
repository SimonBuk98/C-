...
void ParticleSystem::draw()
{
    ...
    for (int i = 0; i < particles.size(); i++)
    {
        cv::circle(img, cv::Point(particles[i].getPosition()(0, 0), particles[i].getPosition()(1, 0)), 5, cv::Scalar(0, 255, 255), -1);
        cv::circle(canvas, cv::Point(particles[i].getPosition()(0, 0), particles[i].getPosition()(1, 0)), 3, cv::Scalar(255, 255, 255), -1);
    }
